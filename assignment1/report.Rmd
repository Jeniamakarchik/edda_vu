...
```{r}
npklm6 <- lm(yield~block + N + K, data=npk)
anova(npklm6)
```

```{r}
npklm7 <- lm(yield~P*N*K, data=npk)
anova(npklm7)
```

We keep additive model for all factors, since pair-wise interactions were insignificant for all the factors. This model is our favorite as it shows also significance of each factor.
